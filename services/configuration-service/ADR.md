# Architecture Decision Records - configuration-service

## ADR-0001: Service ownership

The configuration service (schema catalog, app config) owns the domain described by its schema. All mutations go through
this service; consumers read via its REST API or subscribe to its Kafka events.

## ADR-0002: Data isolation

The service owns configuration_db exclusively. Cross-database references use soft UUID
