...
telemetry service (event ingestion, rollups, retention).
...
Prerequisites: JDK 21, PostgreSQL on localhost:5432, Kafka on localhost:9092.
    ...
    ./gradlew :services:telemetry-service:bootRun

Health: http://localhost:8087/actuator/health
OpenAPI: http://localhost:8087/swagger-ui.html
...
| `spring.r2dbc.url`               | r2dbc:postgresql://localhost:5432/telemetry_db | Database connection |
| `spring.kafka.bootstrap-servers` | localhost:9092                                 | Event bus           |

## Database

Owned schema in `telemetry_db`, managed by Flyway migrations under
