# Architecture Decision Records - scheduler-service

## ADR-0001: Service ownership

The scheduler service (jobs, executions, distributed locks) owns the domain described by its schema. All mutations go through
this service; consumers read via its REST API or subscribe to its Kafka events.

## ADR-0002: Data isolation

The service owns scheduler_db exclusively. Cross-database references use soft UUID
