...
policy engine (rules, violations, escalations).
...
Prerequisites: JDK 21, PostgreSQL on localhost:5432, Kafka on localhost:9092.
    ...
    ./gradlew :services:policy-engine-service:bootRun

Health: http://localhost:8088/actuator/health
OpenAPI: http://localhost:8088/swagger-ui.html
...
| `spring.r2dbc.url` | r2dbc:postgresql://localhost:5432/policy_db | Database connection |
| `spring.kafka.bootstrap-servers` | localhost:9092 | Event bus |

## Database

Owned schema in `policy_db`, managed by Flyway migrations under
