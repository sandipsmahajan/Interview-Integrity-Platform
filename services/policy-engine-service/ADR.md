# Architecture Decision Records - policy-engine-service

## ADR-0001: Service ownership

The policy engine (rules, violations, escalations) owns the domain described by its schema. All mutations go through
this service; consumers read via its REST API or subscribe to its Kafka events.

## ADR-0002: Data isolation

The service owns policy_db exclusively. Cross-database references use soft UUID
